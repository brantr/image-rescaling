...
def main():
    ...
    #hdu_dash = fits.open("DASH-Snippet.fits")
...
#    hdu = fits.open("reprojected_COSMOS_flux_160w-606w-814w.fits")
    ...
    #data_dash = hdu_dash[0].data
    #data_F160W = hdu_F160W[0].data
    #data_F814W = hdu_F814W[0].data
    #data_F606W = hdu_F606W[0].data

    #k_F160W = 0.12786
    #k_F160W = 0.25
    #k_F160W = 0.5  #pretty good
    k_F160W = 0.75
    k_F814W = 2.3066
    k_F606W = 2.75

    #very blue-green
    data_F160W = hdu[0].data[0,:,:]*k_F160W
    data_F814W = hdu[0].data[2,:,:]*k_F814W
    data_F606W = hdu[0].data[1,:,:]*k_F606W
    ...
    data_F160W = np.clip(data_F160W,a_F160W,b_F160W)
    data_F814W = np.clip(data_F814W,a_F814W,b_F814W)
    data_F606W = np.clip(data_F606W,a_F606W,b_F606W)

    print(np.nanmin(data_F160W),np.nanmax(data_F160W))
    print(np.nanmin(data_F814W),np.nanmax(data_F814W))
    print(np.nanmin(data_F606W),np.nanmax(data_F606W))


    #get a rescaled dash image
    #img_dash = rescale_dash(data_dash)

    #get an rgb image from the 
    #F160W, F814W, and F606W images
    img_rgb_init = rescale_cosmos(data_F160W,data_F814W,data_F606W)

    plt.imsave("cosmos.init.png",img_rgb_init)

    img_rgb = rescale_thumbnail(img_rgb_init)


    #save dash img
    plt.imsave("cosmos.png",img_rgb)

    #save dash img
    #plt.imsave("dash.png",img_dash,cmap="gray")
